...
export function resolveParallelism(requestedParallelism, totalQuestions, maxParallelism = 5) {
  const total = Number.isFinite(totalQuestions) ? Math.max(0, Math.floor(totalQuestions)) : 0;
  if (total === 0) return 0;

  const requested = Number.isFinite(requestedParallelism)
    ? Math.max(1, Math.floor(Number(requestedParallelism)))
    : total;

  return Math.max(1, Math.min(total, Math.min(maxParallelism, requested)));
}
